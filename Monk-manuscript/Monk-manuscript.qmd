...
observed recreational angler catch rates [@Cope:2013:ISC].  (THIS PARAGRAPH SEEMS LIKE IT MIGHT BETTER FIT HERE??  or state in a sentence that this work will help future stock assessments as a lead in to the next paragraph??)
...
In order to map(OVERLAY??) the data collected from the onboard observer program with predicted habitat we utilized the bathymetry and backscatter data collected by the California Seafloor Mapping Program (CSMP).
The CSMP formed in response to the network of Marine Protected Area network established in California state waters. The CSMP mapped California state waters (3 nm from shore) north of of Point
Conception ($34^\circ 27^\prime N$) to the California-Oregon border.  Rough (rocky) and smooth (sandy) substrate were identified by CSMP using two rugosity indices, surface:planar area, and vector
ruggedness measure (VRM). The CSMP set a varying VRM threshold for each block. A total of XXX VRM substrate blocks were produced by CSMP, and mosaicked together by authors. 
...
rocky habitat. The cutoff for blue, China and gopher rockfish was six meters, eight meters for vermilion rockfish, 14 meters for black rockfish and 16 meters for brown rockfish. For each species, the
